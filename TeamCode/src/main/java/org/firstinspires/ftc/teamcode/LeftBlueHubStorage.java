...
@Autonomous(name="left blue, hub, park storage", group="Blue", preselectTeleOp = "wroking Teleop")

public class LeftBlueHubStorage extends OpMode {
    ...
    private RightRedHubStorage.State autonomousState = RightRedHubStorage.State.EXIT_START;
        ...
        switch (autonomousState) {
            case EXIT_START:
                ...
                if (rearCm > 35) {
                    leftPower = 0;
                    rightPower = 0;
                    autonomousState = RightRedHubStorage.State.TURN_HUB;
                }
                break;
            case TURN_HUB:
                leftPower = 0.00;
                ...
                if (angle >= 45) {
                    ...
                    rightPower = 0;
                    runtime.reset();
                    autonomousState = RightRedHubStorage.State.DRIVE_FORWARDS;
                }
                break;
            case DRIVE_FORWARDS:
                leftPower = 0.75;
                ...
                if (runtime.milliseconds() >= 500) {
                    runtime.reset();
                    autonomousState = RightRedHubStorage.State.DROP_BLOCK;
                }
                break;
            case DROP_BLOCK:
                clawLeftPower = 1;
                ...
                if (runtime.milliseconds() > 750) {
                    ...
                    clawRightPower = 0;
                    runtime.reset();
                    autonomousState = RightRedHubStorage.State.DRIVE_REVERSE;
                }
                break;
            case DRIVE_REVERSE:
                leftPower = -1;
                ...
                if (runtime.milliseconds() >= 500) {
                    ...
                    rightPower = 0;
                    runtime.reset();
                    autonomousState = RightRedHubStorage.State.TURN_STORAGE;
                }
                break;
            case TURN_STORAGE:
                leftPower = 0.75;
                ...
                if (angle >= 75) {
                    ...
                    armLift.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                    runtime.reset();
                    autonomousState = RightRedHubStorage.State.DRIVE_STORAGE;
                }
                break;
            case DRIVE_STORAGE:
                leftPower = 1;
                ...
                if (runtime.milliseconds() > 5000) {
                    leftPower = 0;
                    rightPower = 0;
                    autonomousState = RightRedHubStorage.State.END;
                }
                break;
            default:
                leftPower = 0;
